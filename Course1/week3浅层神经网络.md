...
<p>当开始训练神经网络时，将权重参数进行随机初始化非常重要。在逻辑回归问题中，把权重参数初始化为零是可行的。但把神经网络的权重参数 W 全部初始化为零并使用梯度下降将无法获得预期的结果</p>
 
<p>但偏置量矩阵 b[1] 的初始值都是 0，不会影响最终结果</p>

<p>W[1] 设置为 0 会导致无论使用什么样的样本进行训练，a[1]1 与 a[1]2 始终是相同的（activation 是一致的）。因为这些隐藏神经元在进行完全相同的计算工作：当你进行反向传播的计算时，由于对称，这些隐藏单元会在同样的条件下被初始化，导致 dz[1]1 和 dz[1]2 不会有差别。<b>所以无论将这个神经网络训练多久，这两个神经网络仍然在使用同样的功能进行运算</b></p>

<p>这并不有用，因为希望<b>不同的隐藏单元能实现不同的功能</b>，只有随机初始化能够解决这样的问题</p>

`W[1] = np.random.randn((2, 2)) * 0.01`

<p>对于 b 来讲，b 不会由于初始值为零而产生对称问题</p>

`b[1] = np.zero((2, 1))`

<p>W 以完成随机初始化，不同的隐藏单元承担不同的计算工作，不会遇到前面说的对称失效问题。</p>

<p>然后可以用同样的方式来将 W[2] 进行随机初始化，b[2] 也依然可以初始化为零</p>
 
<p>关于为什么是 0.01：主要原因是比较喜欢使用非常小的随机初始值。当你使用 tanh 或者 sigmoid 时，如果步长过大，那么你要计算 activation 时：</p>
...
<p>所以当 W 很大时，Z 可能会是一个很大或者很小的数（绝对值很大）。在这种情况下，你可能会发现 tanh 和 sigmoid 斜率很小，意味着梯度下降会非常慢，学习过程也会变慢</p>

<p>最后，有时候会有比 0.01 更合适的数值。当你在训练一个不含有过多隐藏层的神经网络（浅层神经网络）时，0.01 是很合适的。但训练一个复杂的深度神经网络时会使用一个不同的数值</p>
