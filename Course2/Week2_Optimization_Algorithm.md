...
![QQ_1750127968122](https://github.com/user-attachments/assets/42816c31-97e8-4341-8938-c8075f380fb7)

<p>如果画一张二维的图，确实容易像这样出现很多局部最优，直觉会被这些低维图像引导。但是在训练神经网络的时候，代价函数中大部分梯度为零的点实际上并不是像上图的局部最优，而是鞍点</p>

<p>从经验上来看，对于高维空间的函数，如果梯度为零，则在每个方向上都要为凹/凸函数。假设在一个两万维的空间中，如果一个点要成为局部最优，则要在两万个方向上都这样（概率极低）。<b>J 作为一个两万维向量的函数，更有可能遇到鞍点而不是局部最优</b></p>

<p>局部最优不是问题，真正会降低学习速度的实际上是停滞区（Plateaus：导数长时间接近于零的一段区域）</p>
