...
<p>假设正在拟合猫的图片分类器，如果过拟合了，可以增加训练数据，但扩大训练集代价很高，而且有时候就是无法得到更多数据。但可以通过这样处理照片，来扩展训练集。比如把他水平翻转加入到训练集中，也可以随机裁剪图片。增强数据集，做出额外的伪训练样本，这些额外的伪训练样本，能增加的信息量不如全新的，独立的照片多。但确实是一个廉价的方式来获得更多数据（因此可以算作正则化，而且减少了过拟合）</p>

<p>并且通过这样的合成样本，实际上是在告诉你的算法：如果这是一只猫，那它水平翻转后还是一只猫</p>

<p>对于字符识别，也可以 扩增数据集，通过给数字加上随机的旋转和变形，把他们加到你的数据集里面，他们仍然是数字4️(这里可能有点过于扭曲夸张了)</p>
 
![QQ_1745493631354](https://github.com/user-attachments/assets/df43faa2-9abf-4b0b-9e60-ee41c1f00e3a)

<p>所以“数据集扩增”(data augmentation)可以做为一种正则化技术</p>
...
<p>早终止法要做的是，在运行梯度下降时，画一张训练误差的图，可以用训练集的 0-1 分类误差（或者把 J 画出来），他应该是要单调递减。</p>

<p>用 early stopping 要做的是画出这个曲线的同时也画出开发集误差的曲线</p>

![QQ_1745493971692](https://github.com/user-attachments/assets/7c37e5fb-1823-4a16-8354-0041876558f0)

<P>可以看出，通常开发集误差会先下降一段，然后开始增大。找到在哪次迭代附近，神经网络表现的最好，把训练过程挺住</P>

<p>early stopping 的问题是把计算 J 和优化算法的过程结合了，但相比 L2 正则化，不需要大量尝试 lambda 的值以找到最佳值，只需要一次就能找到合适的值</p>

<hr>

<p>Next, let's talk about 优化问题的配制方法来加速训练过程</p>
